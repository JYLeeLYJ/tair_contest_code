...
private:
    ...
    static constexpr size_t VALUE_AREA = NVM_SIZE - KEY_AREA - INDEX_AREA ;
    #else
    static constexpr size_t DRAM_SIZE = 8_GB;
    static constexpr size_t NVM_SIZE = 64_GB ;
    static constexpr size_t KEY_AREA = 14_GB ;
    static constexpr size_t VALUE_AREA = NVM_SIZE - KEY_AREA ;
...
private:
    ...
    bool is_invalid_block(const block_index & block) {
        constexpr auto null_index = decltype(decltype(bucket_infos)::value_type{}.allocator)::null_index;
        return block[0] == null_index || block[1] == null_index || block[2] == null_index || block[3] == null_index;
    }
